...
1. Checking the deployment results\. For more information, see [Monitoring Deployments in CodeDeploy](monitoring.md)\.
...
+ [Deployment Workflow on an Amazon ECS Compute Platform](#deployment-process-workflow-ecs)
...
### Deployment Workflow on an Amazon ECS Compute Platform<a name="deployment-process-workflow-ecs"></a>
...
1. Creating an application by specifying a name that uniquely represents the application revisions you want to deploy and the compute platform for your application\. CodeDeploy uses this name during a deployment to make sure it is referencing the correct deployment components, such as the deployment group, deployment configuration, and application revision\. For more information, see [Create an Application with CodeDeploy](applications-create.md)\.
...
1. Launch Amazon EC2 instances with an IAM instance profile attached\. The IAM instance profile must be attached to an Amazon EC2 instance as it is launched in order for the CodeDeploy agent to verify the identity of the instance\.
...
You can use the CodeDeploy console or the [stop\-deployment](https://docs.aws.amazon.com/cli/latest/reference/deploy/stop-deployment.html) command to stop a deployment\. When you attempt to stop the deployment, one of three things will happen:
